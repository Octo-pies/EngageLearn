...
st.set_page_config(
    page_title="Head Pose & Gaze Detection",
    layout="wide"
)

st.title(" Head Pose & Gaze Detection Dashboard")
st.markdown("### Analyze head movements and gaze direction in real-time using OpenCV & MediaPipe.")
...
videoLink=st.text_input("video URL")
if videoLink:
    st.video(videoLink, format="video/mp4", start_time=0, subtitles=None, end_time=None, loop=True, autoplay=True, muted=False)
...
#data storage
with open('engdata.bin', 'wb') as file:
    pass 
file=open('engdata.bin','ab+')
...
while run and not stop_button_pressed:
...
with col1:
    ...
    st.write("Your webcam feed will display analyzed head pose estimation.")

with col2:
    st.subheader("📊 Gaze Data")
    st.write("Processed gaze data and scores visualization.")
